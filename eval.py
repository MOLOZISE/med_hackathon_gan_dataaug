...
from models import Generator, ViT_Generator
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt', type=str) # 1, 4
    parser.add_argument('--artifacts', type=str, default="train_results/test_4_ntd", help='path to artifacts.')
    ...
    net_ig = ViT_Generator(ngf=64, nz=noise_dim, nc=3, im_size=args.im_size)  # , big=args.big )
    net_ig.to(device)
    ...
    for epoch in [10000 * i for i in range(args.start_iter, args.end_iter + 1)]:
        ...
        del checkpoint
        dist = args.artifacts + '/eval_5000_%d' % (epoch)
